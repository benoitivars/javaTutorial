...
Maintenant que vous avez deux versions de JDK sur votre machine (moi trois), il faut voir laquelle est  celle utilisée ou utilisable par défaut par votre machine avec "java -version" que vous utilisez dans Git Bash.
...
Pour ça, je vais modifier deux choses, la **variable d'environnement**, qui me sert à dire aux programmes qui font tourner Java *quelle version* de Java utiliser, et au besoin le **PATH**, le chemin, qui me sert à dire à dire aux programmes qui peuvent utiliser Java *où* trouver la version à utiliser.
...
![Trouver les paramètres système environnement avec le menu de démarage](images/partie1/parSysAv.png)

Trouver les paramètres système

![Trouver les variables d'environnement](images/partie1/varEnv.png)

Le bouton des variables d'environnement

Une fois dans les variables d'environnement, on va chercher dans les variables systèmes si par hasard, il n'y aurait pas déjà quelque chose en rapport avec un quelconque "path", et là, j'ai eu cette énorme surprise :

![variable path existante](images/partie1/surprise.png)

Et en sélectionnant "Path" et en cliquant sur "modifier", la surprise n'a fait que s'amplifier : sur une variable, j'ai accès à tous les langages d'installés sur ma bécane !

![Les langages installés sont surlignés](images/partie1/languages.png)

En dehors de Python que j'ai aussi sur ma machine qui ne semble pas avoir de variable d'environnement système propre, j'ai sous "path" tous les chemins qui mènent à mes autres langages installés ! Et si vous installez automatiquement les choses comme moi, il y a de fortes chances que vous trouviez tout comme moi sous "path" toutes vos installations de Java !
...
A le mettre tout en haut de la liste
...
Et en fermant Git Bash et en le ré-ouvrant, en utilisant la commande "java -version", vous verrez que la version utilisée par défaut aura changé !
...
Reste à aborder un point : si on doit installer éventuellement d'extensions à VSCode pour faire tourner Java comme il faut, il y a bien une raison, c'est que de base, VSC n'a pas de vocation à faire tourner du Java. Et pour être tout à fait sérieux, il y a des environnements de développement intégrés qui ne s'occupent que de Java, et bien mieux que VSC, dans la mesure où ils sont conçus et calibrés pour. Pour en citer quelques uns, il y a IntelliJ IDEA, Eclipse et NetBeans pour les plus connus.

Si vous ne voulez développer qu'en Java, en faire votre seul langage matin, midi et soir pour le reste de votre vie, envisagez au moins de les tester. Mais si vous venez d'une culture plus web dev avec des tas de fichiers écrits dans différents langages, alors VSC reste indiqué, et est malgré tout, un très bon IDE avec lequel on peut déjà faire beaucoup de choses en Java.
...
*javac  HelloWorld.java*, ce qui va créer dans la zone de gauche le fichier "intermédiaire" *HelloWorld.class* qui est en réalité du Bytecode, le langage intermédiaire entre le Java et le binaire et qui lui sera lu dans la Java Virtual Machine pour être éxécuté. Ici, vous pouvez le voir au dessus de  HelloWorld.java :
...
Je pense qu'on est bien d'accord, ce n'est pas "lisible" par un être humain, même vétéran dans le développement ! Tout au plus peut-on reconnaitre des expressions dans un nuage de caractères incompréhensibles.
...
- Un nom de variable, par convention, s'écrit en camelCase. Vous pouvez choisir de procéder autrement, mais ce serait contraire aux bonnes pratiques du développement Java.
...
Par exemple, on va ajouter la variable de type `char` à notre classe  HelloWorld en décrétant que notre capitaine est une femme, avec la variable `char genreCaptaine = 'F'` :
...
Et là, vous remarquerez une chose, les accents ne passent pas ! Ce n'est pas un si gros soucis que ça. En fait, dans sa compilation en fichier Bytecode, JDK a oublié de prendre en compte l'UTF-8, qui est en quelque sorte une norme d'encodage des caractères qui supporte les accents et peut les afficher une fois un code exécuté. Pour que les choses se passent comme voulue, rien de plus simple.
...
Maintenant, on retente la compilation en javac, mais avec une petite instruction en plus pour bien préciser qu'on utilise lors de la compilation l'encodage UTF-8 comme ceci : `$ javac -encoding UTF-8 HelloWorld.java`et alors le script nous sortira nos lignes de textes avec les accents pris en charge :
...
Les nombres flottants sont ce qu'on appelle les nombres à virgules, qui ne sont pas des entiers, et qu'on peut aussié crire sous forme de fractions. Par exemple, 3,5 peut s'écrire 7/2.
...
Un dernier mot sur notre section consacrée aux variables primitives pour en aborder une qui n'est pas primitive, mais de type **objet**, j'ai nommé la `variable String` ! Dans la mesure où les String sont très souvent utilisés, et qu'ils le sont abondament dans les exemples de ce tuto, je me dois de leur consacrer ici quelques lignes, même s'ils ne sont pas primitifs. Le but ici est de voir leur usage le plus basique.

Les `String` se déclarent comme des variables classiques; et lors du typage de variable, vous devez écrire **String** avec un "S" majuscule. Cette majuscule, apr ailleurs, est ce qui permet de distinguer les variables primitives des variables objet. De plus, leur contenu se met des double-guillemets, `"comme ceci"`, et non dans des guillemets simples comme avec les `char`.

Pour les habitués de JS, ce que vous êtes probablement, vous remarquerez qu'en Java, les guillemets simples ou doubles ne sont pas utilisés de manière indistincte, mais ont des usages précis en fonction de la situation : les simples pour les `char`, les doubles pour les `String`.
...
Dans le fichier "JAVA-EXERCISES.md", vous allez trouver l'énoncé d'un exercice, et dans MardukVolant.java, un code.
...
L'incrémentation fonctionne aussi pour la soustraction, avec la même idée, à chaque fois retirer une valeur.

Les règles syntaxiques sont identiques à celle de l'incrémentation pour l'addition. Ainsi, si vous faites :
...
Nous voici arrivés au dernier opérateur, celui de concaténation, c'est-à-dire le fait **d'associer des chaînes de caractères**, soit différentes `String`. On ne va pas passer beaucoup de temps dessus, car en réalité, nous l'avons déjà beaucoup pratiqué dans les très nombreux exemples que je vous ai donnés depuis le début de ce tuto !
...
C'est le fait d'associer plusieurs `String` entre elles. Si vous regardez par exemple le `Ststem.out.println` associé à la distance parcourure, vous remarquerez que le symbole "+" permet d'associer différentes `String` et même de glisser entre elles les valeurs de variables d'autres types.
...
Or, sous format `String`, si vous additionnez par exemple 5 et 3, vous n'aurez plus 7, mais la concaténation de deux carctères numériques, donc 53, comme vous pouvez le voir dans cet exemple :
...
Si vous regardez bien l'exécution, vous vous rendrez compte que des variables qui étaient de base des `int` sont ici traitées comme des composantes d'une `String`, et que la somme des deux ne fait qu'associer deux `String` composées chacune d'un seul caractère.
...
Dans les versions précédentes de Java, le compilateur créait une `String` intermédiaire à chaque nouvelle 'entrée' de la concaténation, ce qui pouvait poser des problèmes de performance. Dans notre exemple, ça donnait : `"L'addition de 5"`, puis `"L'addition de 5 et "`, puis `"L'addition de 5 et 3"`, et ainsi de suite, au lieu de sortir une String unique, directement.
...
Vous voyez que la variable est directement incluse dans la `String`.
...
- Le `printf` qui signifie "print formated", qui est distinct du `println` que nous utilisons d'habitude qui signifie "print line". On passe ici de lignes classiques à quelque chose de *formaté*.
...
A noter que le formatage permet bien d'inclure plusieurs variables. Il ssuffit alors de les ajouter dans l'ordre où elles apparaissent successivement dans la `String`, après la virgule, comme dans cet exemple :
...
Travailler sur les boucles, conditions, et les objets va nous demander de nous intéresser aux jeux d'accolades et aux lignes pour le moins étranges de par leurs expressions telles que "public class" ou "public static void main", points que nous aborderons très vite dans la seconde partie de ce tutoriel !
